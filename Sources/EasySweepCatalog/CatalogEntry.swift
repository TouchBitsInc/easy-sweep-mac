...
/// grant roots between them. See `GrantRoot`.
...
public enum CatalogRisk: String, Codable, Sendable {
...
/// fields are absent because a contributed file must not be able to set them:
///
/// - whether the one-click clean may take this without the user ticking
///   anything, and
/// - whether removal has to go through a tool such as `simctl` rather than the
///   filesystem.
///
/// Both live in the consuming app, keyed by `id`. A pull request here can add a
/// location; it cannot widen what gets deleted automatically.
public struct CatalogEntry: Codable, Identifiable, Hashable, Sendable {
    /// Stable and permanent. Keys the consuming app's own table of settings, so
    /// renaming one silently drops whatever that app had recorded against it.
    public let id: String
    public let name: String
    /// What the user reads immediately before deleting. States what regenerates
    /// the files and how — never "safe to delete", which `risk` already carries.
    public let detail: String
    /// Tilde-relative, e.g. `~/Library/Caches/Homebrew`. May contain a single
    /// `*` within one path segment; see `PathPattern`.
    public let paths: [String]
    /// Absent means `risky`. Silence is conservative: an entry nobody has
    /// classified should read as the more cautious of the two.
    public let risk: CatalogRisk
    ...
    /// which is safer than a wrong name, because an unknown symbol renders as
    /// nothing at all rather than a placeholder.
    public let symbol: String?
    /// The brand hex exactly as the project publishes it, e.g. `#CB3837`.
    /// Unadjusted — lifting a near-black value for legibility is the consuming
    /// app's business, and doing it here would misreport the brand.
    ...
    public init(from decoder: any Decoder) throws {
        ...
        // Both default rather than being required, so an older build reading a
        // newer catalog degrades instead of failing.
        risk = try container.decodeIfPresent(CatalogRisk.self, forKey: .risk) ?? .risky
