...
struct ApplicationTests {
    ...
    @Test func appHeadingsAndChildrenAreLocalizedIndependently() throws {
        ...
        #expect(application.entries.map(\.id) == ["chrome-cache", "chrome-site-cache", "chrome-ai-model"])
        #expect(application.entries.map(\.risk) == [.safe, .cautious, .cautious])
        #expect(application.entries.map(\.name) == ["Cache", "Site Data", "AI Model"])
